...
for model in models_to_evaluate:
    ...
    for i, row in mean_table.iterrows():
        std_row = std_table.loc[
            i]  # std_table[std_table["Unnamed: 0"]=="covid"]
        # if i==1:
        ...
        print(
            "&", class_map2[i], "& $", round(row["Recall"], 2),
            "\\pm {\scriptstyle", std_row["Recall"], "}$ & $",
            round(row["Precision"],
                  2), "\\pm {\scriptstyle", std_row["Precision"], "}$ & $",
            round(row["F1-score"],
                  2), "\\pm {\scriptstyle", std_row["F1-score"], "}$ & $",
            round(row["Specificity"],
                  2), "\\pm {\scriptstyle", std_row["Specificity"], "}$ & $",
            round(row["MCC"],
                  2), "\\pm {\scriptstyle", std_row["MCC"], "} $ \\\\"
        )
...
for model in vid_models_to_evaluate:
    ...
    for i, row in mean_table.iterrows():
        ...
        print(
            ...
            row["f1-score"], "&", row["Specificity"], "&", row["MCC"], "\\\\"
        )
